...
public class AprioriOld {
	...
	public AprioriOld(File file,int s) {
	...
	public AprioriOld(Vector<Vector<Integer>> baskets,int s) {
...
public AprioriOld(Vector<Vector<Integer>> baskets,int s, BufferedWriter out) {
		...
		if(k>2){
			...
			for(int i=basket.size()-1; i >= 0; i--){
				...
				for(Vector<Integer> itemset :prevCk.keySet()){
					if(prevCk.get(itemset) != 0){//se è frequente
